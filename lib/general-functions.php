...
/*Fetch Provider Attachments*/
function service_finder_cover_image_exists($uid,$type){
	global $wpdb, $service_finder_Tables;
...
global $wpdb, $service_finder_Tables, $service_finder_options;
	...
	foreach($allcities as $city){
		$cityhtml .= '<option value="'.esc_attr($city).'">'.$city.'</option>';
	}
	}else{
...
/*Get User avatar id by its user id*/
function service_finder_get_avatar_by_userid($userid,$size = 'medium'){
global $wpdb,$service_finder_Tables;
...
function service_finder_get_country_list(){
	$country = array(
		...
		'IE' => esc_html__( 'Ireland', 'service-finder' ), //Europe Country
...
/*Get user name by id*/
function service_finder_get_user_name($userid){
...
function service_finder_get_bookingsms_services($bookingid){
global $wpdb, $service_finder_Tables;
$html = '';
$row = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->bookings.' WHERE `id` = %d',$bookingid));
	if(!empty($row)){
...
function service_finder_create_city_term($cityname = '',$countryname = ''){
...
function service_finder_cities_by_country(){
	...
	if(!empty($cities)){
		foreach($cities as $term){
			$citydropdown .= '<option value="'.esc_attr($term->name).'">'.$term->name.'</option>';
		}
	}
	echo $citydropdown;
	exit;
...
/*Get user image url*/
function service_finder_get_user_profile_image($avatar_id,$size = 'thumbnail'){
$src = '';
...
function service_finder_get_experience($user_id){
	global $wpdb,$service_finder_Tables;
...
function service_finder_get_certificates($user_id){
	global $wpdb,$service_finder_Tables;
...
function service_finder_get_qualifications($user_id){
	global $wpdb,$service_finder_Tables;
...
function service_finder_get_alllanguages(){
	...
	$lng['ca'] = esc_html__( 'Catalan', 'service-finder' );
...
/*Get job quote price*/
function service_finder_get_job_quote_price($providerid,$jobid){
...
function service_finder_check_identity_approved($uid){
	global $wpdb, $service_finder_Tables, $service_finder_options;
...
function service_finder_comment_inserted($comment_id, $comment_object) {
...
function service_finder_get_provider_paypal_email($userid)
{
	if($userid > 0)
	{
...
function service_finder_user_has_capability($capability,$userid = 0)
{
...
function service_finder_get_image_url($attachmentid,$size = 'thumbnail')
{
	if($attachmentid > 0)
	{
...
/* Get attachment url by attachment id */
function service_finder_get_user_coverimage($userid,$size = 'thumbnail')
{
...
function service_finder_create_default_timeslots($uid){
global $wpdb, $service_finder_Tables, $service_finder_options;
...
function service_finder_create_default_starttime($uid){
global $wpdb, $service_finder_Tables, $service_finder_options;
...
function service_finder_get_data( $obj, $key, $default = '' )
{
	if( !$obj )
	{
		return false;
...
/* Get quote info */
function service_finder_get_quote_info($quoteid){
	global $wpdb, $service_finder_Tables, $service_finder_options;
...
function service_finder_get_related_quote_providers($quoteid){
	global $wpdb, $service_finder_Tables, $service_finder_options;
...
function service_finder_has_sent_invitation($jobid,$providerid) 
{
	global $wpdb,$service_finder_options,$service_finder_Tables;
...
function service_finder_has_sent_job_notification($jobid,$providerid) 
{
...
function service_finder_has_applied_for_job($jobid,$providerid) 
{
	$job_applications = get_post_meta($jobid,'job_applications',true);
...
/*Get provider info */
function service_finder_get_provier_info($providerid){
	global $wpdb, $service_finder_Tables;
...
function service_finder_get_total_reviews($providerid){
	global $wpdb, $service_finder_Tables, $service_finder_options;
...
function service_finder_contact_number_is_accessible($providerid){
global $service_finder_options;
...
/*Check pay only admin fee is on/off*/
function service_finder_get_primary_category_tag($providerid){
	global $service_finder_options;
	$html = '';
	if(get_user_meta($providerid,'primary_category',true) != '')
	{
...
/*Get Job Categories*/
function service_finder_get_job_categories($jobid){
...
function service_finder_get_service_location($bookingid){
	global $wpdb, $service_finder_Tables, $service_finder_options;
