...
$state = service_finder_get_data($_REQUEST,'state');
...
$advancesearchview = (isset($service_finder_options['advance-search-view'])) ? esc_attr($service_finder_options['advance-search-view']) : '';
...
case 2:
	$fieldclass = 'col-md-9 search-col-1';
	$submitclass = 'col-md-3 search-col-1';
	break;
case 3:
...
case 6:
	...
	break;
case 7:
	$fieldclass = 'col-md-2 col-sm-4 search-col-7';
	$submitclass = 'col-md-2 col-md-4 col-sm-12 search-col-7';
	break;
case 8:
	$fieldclass = 'col-md-2 col-sm-4 search-col-8';
	$submitclass = 'col-md-2 col-md-4 col-sm-12 search-col-8';
	break;		
}
?>
...
$html = '<form class="clearfix search-providers" method="get" action="'.home_url('/').'">
  ...
  $html .= '<div class="btn-advace-wrap clear">
                     <a class="btn btn-advace-search openadvsrh" ><i class="fa '.sanitize_html_class($arrowclass).'"></i></a>
                    </div>
  ';
  }
  ...
  $html .= '<div class="'.$fieldclass.'">
    <div class="type-keyword">
      <input type="text" value="'.esc_attr($searchAddress).'" placeholder="'.esc_html__('Adres', 'service-finder').'" id="searchAddress" name="searchAddress" class="form-control sf-form-control">
	  '.$autolocation.'
    </div>
  </div>';
  }
  
  $showemptycategory = (!empty($service_finder_options['show-empty-category'])) ? $service_finder_options['show-empty-category'] : '';
  ...
  $html .= '<div class="'.$fieldclass.'">
    <div class="select-city">';
				$html .= '<select class="sf-select-box form-control sf-form-control" data-live-search="true" name="city" id="city" title="'.esc_html__('Miasto','service-finder').'" data-header="'.esc_html__('Select a City','service-finder').'">';
			   ...
			   $branchcities = $wpdb->get_results($wpdb->prepare("select DISTINCT city from ".$service_finder_Tables->branches." WHERE country = '%s' ORDER BY `city`",$country));
				...
				sort($allcities);	
					...
					foreach($allcities as $city){
						...
						$html .= '<option '.$select.' value="'.esc_attr($city).'">'.$city.'</option>';
  ...
  if(service_finder_get_data($service_finder_options,'search-zipcode')){
  ob_start();
  ?>
  	<div class="<?php echo $fieldclass; ?>">
  ...
  $html .= '<div class="sf-advace-search clearfix clear '.sanitize_html_class($hiddenclass).'">';
